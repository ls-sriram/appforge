...
- `Block` — use `View`, `XStack`, `YStack` with raw Tamagui props
- `Col`, `Row`, `Card`, `Screen`, `Rule`, `Divider`, `AbsLayer` (all from old `SStack`) — removed; use `XStack`/`YStack`/`View`
- `shared panels` — removed; compose `View`/`YStack` inline per screen
