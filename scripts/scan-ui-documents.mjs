...
function updateRegistry() {
  ...
  const appIds = docFiles.map((f) => f.replace("ui-documents.", "").replace(".ts", ""));

  function toCamel(id) {
    return id.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
  }

  const imports = appIds
    .map((id) => `import { SCANNED_UI_DOCUMENTS as ${toCamel(id)}Docs } from "./ui-documents.${id}";`)
    .join("\n");

  const registryEntries = appIds.map((id) => `  "${id}": ${toCamel(id)}Docs,`).join("\n");
  const nameEntries = appIds
    .map((id) => `  "${id}": ${JSON.stringify(manifest.apps[id]?.displayName ?? id)},`)
    .join("\n");

  const content = `// AUTO-GENERATED by scripts/scan-ui-documents.mjs — do not edit manually.
...
`;
  ...
  const registryPath = path.join(generatedDir, "ui-documents.registry.ts");
  fs.writeFileSync(registryPath, content, "utf-8");
  console.log(`[scan] Updated registry: ${path.relative(ROOT, registryPath)} (${appIds.length} apps: ${appIds.join(", ")})`);
}
