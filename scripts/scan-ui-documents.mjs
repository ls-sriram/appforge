...
// Scan output lives in appforge-site (the consumer). Types are local to that repo.
const APPFORGE_SITE_ROOT = path.resolve(ROOT, "../appforge-site");
const GENERATED_DIR = path.join(APPFORGE_SITE_ROOT, "generated");
...
import type { UiDocument } from "../features/ui-visualizer/domain/ui-document.types";
...
function generateBlocksTs(appId, blocks) {
  ...
  return `// AUTO-GENERATED by scripts/scan-ui-documents.mjs — do not edit manually.
// Run: node scripts/scan-ui-documents.mjs <appId>
import type { UiBlock } from "../features/ui-visualizer/domain/ui-document.types";
...
const outPath = path.join(GENERATED_DIR, `ui-documents.${appId}.ts`);
fs.writeFileSync(outPath, generateTs(documents), "utf-8");
log(`[scan] Wrote ${path.relative(ROOT, outPath)} (${documents.length} documents)`);

const blocksOutPath = path.join(GENERATED_DIR, `ui-blocks.${appId}.ts`);
